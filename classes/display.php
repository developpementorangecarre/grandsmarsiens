...
class display
{
	...
	function displayManageTable($tableprefix,$tablename,$tableheaders,$tablefields,$wherefield,$wheredata,$orderby)
	{
		...
		//if ($tablename=='suppliers')
			...
			$ventes="<a href=\"../../reports/supplier.php?vendeur=";
			$ventes2="\"><font color='$this->rowcolor_link'>Vente</font></a>";
		...
		while($row=mysql_fetch_assoc($result))
		{
			...
			echo "<td align='center'>\n<a href=\"form_$tablename.php?action=update&id=$row[id]\"><font color='$this->rowcolor_link'>{$this->lang->update}</font></a></td>
				 <td align='center'>\n<a href=\"javascript:decision('{$this->lang->confirmDelete} $table {$this->lang->table}?','process_form_$tablename.php?action=delete&id=$row[id]')\"><font color='$this->rowcolor_link'>{$this->lang->delete}</font></a></td><td>$ventes$row[id]$ventes2</td>\n</tr>\n\n";
		}
			echo '</table>'."\n";
	}
